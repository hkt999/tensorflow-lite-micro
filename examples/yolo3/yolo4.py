...
def YOLO(input_layer, NUM_CLASS, model='yolov4', is_tiny=False, wyze=False):
    if wyze:
        print("#############################3 Using wyze YOLO ########################")
        return YOLOv3_tiny_wyze(input_layer,NUM_CLASS)
...
def YOLOv3_tiny_wyze(input_layer, NUM_CLASS):
    route_1, conv = backbone.darknet53_tiny_wyze(input_layer)
    #print(conv)
    #print(route_1)
    conv_lbbox = common.convolutional(conv, (1, 1, 64, 3 * (NUM_CLASS + 5)), activate=False, bn=False)

    conv = common.upsample(conv)
    #print(conv)
    conv = tf.concat([conv, route_1], axis=-1)

    conv_mobj_branch = common.convolutional(conv, (3, 3, 128, 64))
    conv_mbbox = common.convolutional(conv_mobj_branch, (1, 1, 64, 3 * (NUM_CLASS + 5)), activate=False, bn=False)
